...
PYBIND11_MODULE(_mlir_backend, m) {
    ...
    py::class_<mlir_edsl::MLIRCompiler>(m, "MLIRCompiler")
        .def(py::init<>())
        .def("compile_function", &mlir_edsl::MLIRCompiler::compileFunction,
        ...
        .def("get_function_signature", [](mlir_edsl::MLIRCompiler& self, const std::string& name) {
        ...
        .def("set_optimization_level", [](mlir_edsl::MLIRCompiler& self, int level) {
            mlir_edsl::MLIRCompiler::OptLevel opt;
            if (level == 0) opt = mlir_edsl::MLIRCompiler::OptLevel::O0;
            else if (level == 2) opt = mlir_edsl::MLIRCompiler::OptLevel::O2;
            else if (level == 3) opt = mlir_edsl::MLIRCompiler::OptLevel::O3;
            else opt = mlir_edsl::MLIRCompiler::OptLevel::O2;
